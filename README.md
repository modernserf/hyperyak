...
hyperbuzz
```
A more generalized version of this is transclusion -- embedding one document inside another. Its the same principle as hyperlinks, except it embeds (or transcludes) one document into another instead of just referencing it.
```
...
```
But you can do a lot more with transclusion. For example, it provides a basis for collaboration: transclusions can be used for annotations...
